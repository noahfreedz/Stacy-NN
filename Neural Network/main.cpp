...
void NeuralNetwork::print_network() {
    ...
    // Print weights
    for (size_t i = 0; i < network_weights.size(); ++i) {
    ...
    // Print biases
    for (size_t i = 0; i < network_biases.size(); ++i) {
...
void NeuralNetwork::export_network(const std::string &filename) {
    ...
    file << "WEIGHTS\n";
    for (const auto &matrix : network_weights) {
    ...
    file << "BIASES\n";
    for (const auto &vector : network_biases) {
        file << vector.size() << "\n";
